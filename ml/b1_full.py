"""B1 (full): stream the BigEarthNet S2 and S1 archives into LMDB, never unpacking them to disk (ML_PLAN phase 1).
    ...
    .venv-ml/Scripts/python.exe ml/b1_full.py ingest --modality s2      # the whole archive into data/b1_full/s2
    .venv-ml/Scripts/python.exe ml/b1_full.py ingest --modality s1
...
name with an `s1:` prefix and counted. They are kept (owner decision 2026-09-22): no annotations use them, but
they are real co-registered S1/S2 pairs, which B9's contrastive training needs; the prefix is the flag that lets
annotation-based datasets skip them.

`ingest` stores every complete patch of an archive and writes ml/b1_full_ingest_<modality>.json with the counts.
It applies no split and no filter: the official split with the 2-cell buffer, the 5%-cloud cap and its
pairs_in/pairs_kept report, and the box rows are an index built over the stored pool afterwards, so changing a rule
never means re-reading 117 GB of archives.

On Windows an LMDB data file is as large as its map size from the start, so the map begins small and grows in
fixed GROW_BYTES steps when full (doubling would take a 64 GiB store to a 128 GiB file). Resumable: a patch whose
key is already present is skipped before it is decoded.
"""
import argparse
import json
import sys
...
class Store:
    """An LMDB environment whose map grows by GROW_BYTES when full (the file is the map size on Windows)."""
    ...
    def commit(self):
        while self.batch:
            try:
                ...
                self.env.set_mapsize(self.env.info()["map_size"] + GROW_BYTES)
...
def run(modality, dest, limit=None, progress_every=1000):
    """Streams one archive into the store at `dest`, stopping after `limit` patches (None: the whole archive)."""
    archive = ARCHIVES[modality]
    archive_bytes = archive.stat().st_size
    mapping = s1_to_patch() if modality == "s1" else None
    store = Store(dest)
    st = {"stored": 0, "skipped": 0, "unmatched": 0, "value_bytes": 0, "compressed": 0}
    ...
    for folder, blobs, compressed in patches(archive, modality):
        ...
        if store.has(key):
            ...
            store.put(key, value)
            st["value_bytes"] += len(value)
            st["stored"] += 1
        n = st["stored"] + st["skipped"]
        if n % progress_every == 0:
            print(f"{modality}: {n:,} patches, {compressed / archive_bytes:.1%} of the archive, "
                  f"{time.perf_counter() - start:.0f} s", flush=True)
        if limit is not None and n >= limit:
            break
    store.commit()
    st["seconds"] = time.perf_counter() - start
    st["used"] = store.used_bytes()
    st["entries"] = store.count()
    store.close()
    return st
...
def cmd_measure(args):
    ...
    st = run(modality, MEASURE_DIR / modality, args.limit)
    stored, skipped, unmatched, value_bytes = st["stored"], st["skipped"], st["unmatched"], st["value_bytes"]
    seconds, used, compressed = st["seconds"], st["used"], st["compressed"]
...
def cmd_ingest(args):
    modality, archive = args.modality, ARCHIVES[args.modality]
    dest = POOL_DIR / modality
    st = run(modality, dest, progress_every=10000)
    report = {
        "modality": modality,
        "archive": str(archive.relative_to(ROOT)),
        "archive_bytes": archive.stat().st_size,
        "store": str(dest.relative_to(ROOT)),
        "bands": list(BANDS[modality]),
        "dtype": np.dtype(DTYPES[modality]).name,
        "shape": [len(BANDS[modality]), SIZE, SIZE],
        "entries": st["entries"],
        "stored_this_run": st["stored"],
        "skipped_already_present": st["skipped"],
        "s1_without_parquet_row": st["unmatched"] if modality == "s1" else None,
        "s1_without_parquet_row_note": ("kept under an 's1:' key: real S1/S2 pairs for B9, no annotations"
                                        if modality == "s1" else None),
        "lmdb_used_bytes": st["used"],
        "seconds_this_run": round(st["seconds"], 1),
        "finished_on": time.strftime("%Y-%m-%d %H:%M"),
    }
    out = Path(__file__).resolve().parent / f"b1_full_ingest_{modality}.json"
    out.write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
    print(json.dumps(report, indent=2))
...
def main():
    ...
    i = sub.add_parser("ingest", help="stream the whole archive into data/b1_full/<modality> (resumable)")
    i.add_argument("--modality", choices=sorted(ARCHIVES), required=True)
