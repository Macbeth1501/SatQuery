...
> **B1 status: thin slice DONE 2026-09-20** (`ml/b1_slice.py`, `ml/b1_slice_report.json`): 1,995 train and 206 validation patches, 17,955 + 1,854 examples, official split labels with a 2-cell spatial buffer. Two departures from the row below, both forced by the data: the official split is patch-level, not geographic (52 of 54 tiles appear in several splits), and the "drop the bottom quartile" quality filter is degenerate here (75% of patches score exactly zero), so an absolute cloud-like cap of 5% replaced it. Bounding-box rows were left out, since B6 grounding is trained separately.
> **B1 status: full ingestion DONE 2026-09-23.** `ml/b1_full.py ingest` streamed both archives into LMDB, never unpacking to disk: S2 549,488 patches / 61 GB, S1 549,488 entries / 32 GB (about 93 GiB total, close to the 96 GiB projection). **85,444 patches in each archive (15.5%) have no BigEarthNet.txt row at all** -- confirmed to be the same set both ways, i.e. these patches were simply never part of BigEarthNet.txt's curated annotation set, not a join gap; kept regardless (owner decision), as real S1/S2 pairs for B9. `ml/b1_index.py split` applied the same official-split-plus-buffer rule at full scale over the parquet: 464,044 total annotated patches, 199,279 usable `train` after buffering 29,835 (13%) out against `validation`/`bench`; `test` is not buffered against, matching `b1_slice.py`'s own choice, flagged since nothing has scored against `test` yet. **The `[x0 y0, x1 y1]` box convention is confirmed**, per the plan's requirement before B6 trains: `ml/b1_boxes.py verify` checked 20,000 `point`-category rows and found the box, read exactly as written (x horizontal, y vertical, origin top-left, no swap or flip), contains its named point 100% of the time, with its own two corners always already given top-left-first; every alternative axis reading tested is far below, and a visual spot-check (`ml/b1_boxes.py draw`) confirms boxes land on the described feature. `ml/b1_index.py quality` (the 5%-cloud cap): of 549,488 patches, 81,987 dropped for cloud-likeness, 0 for NoData (`data/b1_full/quality_report.json`). `ml/b1_index.py boxes`: all 2,205,686 box rows map to a stored S2 patch (`data/b1_full/boxes.jsonl`), 1,582 of them on the bench-subset patches used for the visual spot-check -- matching the count the plan itself had already cited for B6, confirming that citation was this same intersection.
...
> **B2 status: DONE 2026-09-22** (`ml/serve_vqa.py`, checked by `ml/b2_check.py`, results in `data/b2_eval/`). Run 2 and run 3 stay resident on the one 4-bit backbone; `/health`, `/adapters` and `/infer` with task tokens (the shared helper is in `ml/b5_common.py`). Switching adapters costs nothing measurable (run2->run3 426.8 ms vs run3->run3 426.3 ms median), and run 2's 23 live-demo answers are bit-identical to the single-adapter server's. The backend names the adapter with `SATQUERY_VQA_MODEL_ADAPTER` (default `run2`). Still in-process per server, one generation at a time on the 4 GB card; no separate serving container.
...
> **B4 status: DONE 2026-09-23.** `backend/app/services/modality_model.py` (12 dtype-independent features, logistic regression served from exported JSON coefficients, so the backend gains no dependency) and `ml/b4_modality.py` (fits in `.venv-ml`, scores on the official held-out `test` split across five renderings against the always-optical baseline: 6,000 train / 3,000 test patches, half optical half SAR). **It beats the baseline (50%) on every rendering, with no fitting on test data:** native and float-linear 100.0%, 8-bit RGB 93.5%, 8-bit grey (with or without 3-channel replication) 85.6%; overall 92.9%. Feature extraction costs 10.2 ms per call, well under the 100 ms budget. Consulted only when tags and file name both say nothing, and silent below `SATQUERY_MODALITY_MIN_PROBABILITY` (default 0.9); a raising classifier is caught and the old default stands. `backend/app/services/modality_model.json` is the fitted, committed model.
...
> **B8 status: the functions are DONE 2026-09-22** (`ml/b8_domain_gap.py`, 8 tests); the per-adapter ablation tables are phase 7 and not started. Sensor values are quoted and cited in `SPECS` (SentiWiki for S1 IW GRDH ENL 4.4; eoPortal for RISAT-1 single look and Cartosat-2S 0.65 m / 2 m). Cartosat's bit depth and radiometric histogram are flagged as unavailable, so `match_histogram` refuses to run without a real Cartosat reference rather than inventing one. **Appendix A #13 resolved:** normalisation applies to every specialist call, at C2's shared entry point (not wired, since C2 does not exist yet).
...
> **C0 status: DONE 2026-09-22.** None of the three imports or calls the scenario engine any more, and
> `tests/test_scenario_isolation.py` asserts they cannot start again. The router is the single place that reads
> it: it passes `demo=` to the detector and verbalizer and `demo_confidence` to the scorer, and reads nothing at
> all on a path a real model serves, so scripted content cannot re-rate or reword a real answer. Without demo
> content the detector and verbalizer use their own rule-based output. Demo output is unchanged
...
| 13 | Domain-gap normalization hook: fusion-only or all specialists | B9.4, M6.4.1 | **Resolved 2026-09-22** — all specialists, at C2's shared entry point, per the approved `docs/ML_PLAN.md`; the function exists (`ml/b8_domain_gap.py`), the wiring waits for C2 |
